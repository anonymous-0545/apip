...
#basePath = '/mnt/data/MONN/'

def toAbsolute(path):
    return os.path.join(basePath, path)

print(f'Current working directory: {basePath}')
...
class ProteinDataset(torch.utils.data.Dataset):
    def __init__(self, data_pack, num_values=9):
        self.num_values = num_values
        assert len(data_pack) == num_values
    ...
    def __getitem__(self, idx):
        ...
        return [self.data_pack[data_idx][idx] for data_idx in range(self.num_values)]
...
def batch_data_process(data, logp_flag=False, extended_flag=False):

    if logp_flag:
        if extended_flag:
            vertex, edge, atom_adj, bond_adj, nbs, sequence, aff_label, pairwise_mask, pairwise_label, cid, pid, log_p = zip(
                *data)
        else:
            vertex, edge, atom_adj, bond_adj, nbs, sequence, aff_label, pairwise_mask, pairwise_label, log_p = zip(
                *data)
    else:
    ...
    if logp_flag:
        ...
        if extended_flag:
            return vertex_mask, vertex, edge, atom_adj, bond_adj, nbs_mask, seq_mask, sequence, aff_label, pairwise_mask, pairwise_label, cid, pid, log_p
        else:
            return vertex_mask, vertex, edge, atom_adj, bond_adj, nbs_mask, seq_mask, sequence, aff_label, pairwise_mask, pairwise_label, log_p
    else:
...
# load data
def data_from_index(data_pack, idx_list, logp=False):
    fa, fb, anb, bnb, nbs_mat, seq_input = [data_pack[i][idx_list] for i in range(6)]
    if len(data_pack[6]) == 0:
        aff_label = data_pack[9][idx_list].astype(float).reshape(-1,1)
    else:
    ...
    out = [fa, fb, anb, bnb, nbs_mat, seq_input, aff_label, pairwise_mask, pairwise_label]
    if logp:
       assert len(data_pack) >= 11
       out.append(data_pack[11][idx_list])
    return out

def data_from_index_all(data_pack, idx_list, logp=False):
    fa, fb, anb, bnb, nbs_mat, seq_input = [data_pack[i][idx_list] for i in range(6)]
    if len(data_pack[6]) == 0:
        aff_label = data_pack[9][idx_list].astype(float).reshape(-1,1)
    else:
    ...
    pairwise_mask = data_pack[9][idx_list].astype(float).reshape(-1,1)
    pairwise_label = data_pack[10][idx_list]
    out = [fa, fb, anb, bnb, nbs_mat, seq_input, aff_label, pairwise_mask, pairwise_label, cid, pid]
    if logp:
       assert len(data_pack) >= 11
       out.append(data_pack[11][idx_list])
    return out
...
def split_train_test_clusters(measure, clu_thre, n_fold, deleted_proteins=None):
    ...
    # deleted_num = 0
    # for key in deleted_proteins:
    #     if key in P_cluster_dict:
    #         deleted_num += 1
    #         del P_cluster_dict[key]
    #     if key in C_cluster_dict:
    #         deleted_num += 1
    #         del C_cluster_dict[key]
    #
    # print(deleted_num)
...
def load_test_data(max_sequence_length=512):
    # load data
    with open(toAbsolute('data/independent_all_combined_input_IC50'), 'rb') as f:
        data_pack = load_pickle(f)

    data_pack[7] = fix_list(data_pack[7])
    data_pack[8] = fix_list(data_pack[8])
    print(len(data_pack))
    #print([type(data_pack[0]) for i in range(len(data_pack))])
    ...
    if max_sequence_length > 0:
        ...
        print(f'Max sequence: {max_sequence}, max vertex: {max_vertex}')
    # print('0')
    # import plotly.express as px
    # fig = px.histogram(length_list)
    # fig.write_html("./protein_histogram.html")
    # print('should stop')
...
def load_data_all(measure, max_sequence_length=512):
    with open(toAbsolute('data/pdbbind_all_combined_input_'+measure) + '_logp','rb') as f:
        data_pack = load_pickle(f)
...
def load_data(measure, setting, clu_thre, n_fold, max_sequence_length=512, log_p=False, num_pairs=-1):
    # load data
    ...
    if log_p:
        ...
        name_append = ''
    with open(toAbsolute('data/pdbbind_all_combined_input_'+measure+name_append),'rb') as f:
        data_pack = load_pickle(f)
...
def loading_emb(measure):
    ...
    f = open(toAbsolute('data/pdbbind_all_atom_dict_'+measure),'rb')
    atom_dict = load_pickle(f)
    atom_dict = fix_dict(atom_dict)
    f.close()

    f = open(toAbsolute('data/pdbbind_all_bond_dict_'+measure),'rb')
    bond_dict = load_pickle(f)
    bond_dict = fix_dict(bond_dict)
    f.close()

    f = open(toAbsolute('data/pdbbind_all_word_dict_'+measure),'rb')
...
class Weighted_Masked_BCELoss(nn.Module):
    def __init__(self):
        super(Weighted_Masked_BCELoss, self).__init__()
        # self.criterion = nn.BCELoss(reduce=False)

    def forward(self, pred, label, pairwise_mask, vertex_mask, seq_mask):
        batch_size = pred.size(0)
        # pos_weight, neg_weight = [0.9, 0.1]
        pos_weight, neg_weight = [0.999, 0.001]

        output = torch.clamp(pred, min=1e-7, max=1 - 1e-7)
        loss_all = pos_weight * (label * torch.log(output)) + neg_weight * ((1 - label) * torch.log(1 - output))
